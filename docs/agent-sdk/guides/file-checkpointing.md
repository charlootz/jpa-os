# Rewind file changes with checkpointing - Claude Docs
...
**Solution**: Make sure you've set the environment variable (see [Set the environment variable](https://platform.claude.com/docs/en/agent-sdk/file-checkpointing#set-the-environment-variable)), then use the pattern shown in the examples: capture the first user message UUID, complete the session fully, then resume with an empty prompt and call `rewindFiles()` once.
