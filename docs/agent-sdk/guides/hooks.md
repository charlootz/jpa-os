...
- **Values** are arrays of [matchers](https://platform.claude.com/docs/en/agent-sdk/hooks#matchers), each containing an optional filter pattern and your [callback functions](https://platform.claude.com/docs/en/agent-sdk/hooks#callback-function-inputs)

Your hook callback functions receive [input data](https://platform.claude.com/docs/en/agent-sdk/hooks#input-data) about the event and return a [response](https://platform.claude.com/docs/en/agent-sdk/hooks#callback-outputs) so the agent knows to allow, block, or modify the operation.
...
1. **Input data** (`dict` / `HookInput`): Event details. See [input data](https://platform.claude.com/docs/en/agent-sdk/hooks#input-data) for fields
...
- Hooks may not fire when the agent hits the [`max_turns`](https://platform.claude.com/docs/en/agent-sdk/python#configuration-options) limit because the session ends before hooks can execute
